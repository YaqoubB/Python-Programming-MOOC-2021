...
def most_goals(players: list):
    return str((max(players, key = lambda player: player.goals)).nimi)

def most_points(players: list):
    most = max(players, key = lambda player: (int(player.goals) + int(player.passes)))
    return (most.nimi , most.number)
